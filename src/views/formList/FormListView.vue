<template>
  <div class="list-page">
    ...
    <div class="list-content">
      ...
      <EmptyState v-else-if="store.filteredForms.length === 0 && !store.keyword" text="还没有表单" desc="创建你的第一个表单开始吧" action-text="新建表单" @action="handleCreate" />
      <div v-else class="table-wrap">
        <el-table :data="store.filteredForms" stripe>
          <el-table-column prop="title" label="表单名称" min-width="200">
            <template #default="{ row }"><span class="form-name" @click="handleEdit(row.id)">{{ row.title || '未命名表单' }}</span></template>
          </el-table-column>
...
function getSubmissionCount(formId: string): number {
  try { return JSON.parse(localStorage.getItem(`formcenter_submissions_${formId}`) ?? '[]').length } catch { return 0 }
}
function formatTime(ts: number): string {
  return new Date(ts).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })
}
function statusType(s: string) { return s === 'open' ? 'success' : s === 'closed' ? 'danger' : 'warning' }
function statusLabel(s: string) { return s === 'open' ? '收集中' : s === 'closed' ? '已关闭' : '草稿' }

function handleCreate() { router.push('/formDesigner') }
function handleEdit(id: string) { router.push(`/formDesigner/${id}`) }
function handleFill(id: string) { window.open(`/fill/${id}`) }
async function handleCopyLink(id: string) {
  const ok = await copyToClipboard(`${window.location.origin}/fill/${id}`)
  ElMessage[ok ? 'success' : 'error'](ok ? '链接已复制' : '复制失败')
}
function handleData(id: string) { router.push(`/forms/${id}/responses`) }
async function handleDelete(row: FormSchema) {
  await ElMessageBox.confirm(`删除后将清除「${row.title}」的表单定义、所有提交数据和草稿。此操作不可恢复。`, '确认删除', { confirmButtonText: '确认删除', cancelButtonText: '取消', type: 'warning' })
  store.remove(row.id)
...
.page-title { font-size: 30px; font-weight: 600; }
.toolbar-actions { display: flex; gap: 12px; }
.search-bar { margin-bottom: 20px; max-width: 360px; }
.form-name { color: var(--color-primary); cursor: pointer; }
.form-name:hover { color: var(--color-primary-hover); }
.no-result { text-align: center; color: var(--color-text-muted); padding: 40px 0; }
.template-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
.template-card { border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 20px; cursor: pointer; transition: all .15s; }
.template-card:hover { border-color: var(--color-primary); box-shadow: var(--shadow-sm); }
