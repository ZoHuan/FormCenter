  ...
  <div class="list-page">
    ...
    <div class="list-content">
      ...
      <div v-else class="form-cards">
        <div
          v-for="(row, idx) in store.filteredForms"
          :key="row.id"
          class="form-card"
          ...
          <div class="fc-body" @click="handleEdit(row.id)">
            <div class="fc-header">
              ...
              <div class="fc-actions" @click.stop>
                <el-button v-if="row.status === 'draft'" size="small" @click="handleEdit(row.id)"
                  ><Pencil :size="14" />编辑</el-button
                >
                <el-button v-if="row.status === 'draft'" size="small" type="primary" @click="handlePublish(row)"
                  ><Send :size="14" />发布</el-button
                >
                <el-button v-if="row.status === 'open'" size="small" @click="handleData(row.id)"
                  ><Table2 :size="14" />数据</el-button
                >
                <el-button v-if="row.status === 'closed'" size="small" @click="handleData(row.id)"
                  ><Table2 :size="14" />数据</el-button
                >
                <el-dropdown trigger="click" @command="(cmd: string) => handleCardAction(cmd, row)">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item v-if="row.status === 'open' || row.status === 'closed'" command="edit"
                        >编辑</el-dropdown-item
                      >
        ...
        <p v-if="store.filteredForms.length === 0 && store.keyword" class="no-result">
          没有找到「{{ store.keyword }}」相关的表单
        </p>
      </div>
    </div>
...
const tplIcons: Record<string, any> = {
  leave: CalendarDays,
  event: PartyPopper,
  survey: BarChart3,
