...
function toggle(cat: string) { if (openGroups.value.has(cat)) openGroups.value.delete(cat); else openGroups.value.add(cat) }
function onDragStart(e: DragEvent, type: ComponentType) { e.dataTransfer?.setData('componentType', type) }
const labels: Record<string, string> = {
  ...
  date: '日期', 'date-range': '日期区间', image: '文件附件', singleImage: '图片', signature: '签名',
  rate: '评分', table: '表格', 'cross-table': '交叉表', relation: '关联查询', commitment: '承诺说明',
  region: '行政区划', 'map-location': '地图定位', 'user-tree': '用户选择', 'org-tree': '组织选择',
  'signature-name': '签名人', QRCode: '二维码',
}
...
.group-title {
  ...
  &:hover { background: var(--color-canvas); border-radius: 4px; }
}

.arrow { font-size: 10px; color: var(--color-text-muted); transition: transform .2s; display: inline-block; }
.arrow.open { transform: rotate(90deg); }

.palette-item {
  height: 40px; display: flex; align-items: center;
  padding: 0 12px; font-size: 13px; cursor: grab;
  border-radius: var(--radius-md); margin: 2px 0;
  transition: background .15s;
