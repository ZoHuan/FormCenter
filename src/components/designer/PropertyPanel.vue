  ...
  <div class="prop-panel">
    ...
    <div class="section">
      <div class="section-header" @click="toggleSection('basic')">
        <span class="section-arrow" :class="{ open: openSections.basic }">▸</span>
        基础
      </div>
      <div v-show="openSections.basic" class="section-body">
        ...
        <div class="prop-row">
          ...
          <el-select v-model="comp.colspan" size="small" @change="emitUpdate">
            ...
            <el-option :value="3" label="三列" />
            <el-option :value="4" label="长标题" />
          </el-select>
        </div>
      </div>
    </div>
    ...
    <div v-if="hasOptions" class="section">
      ...
      <div v-show="openSections.options" class="section-body">
        <div class="prop-row"><label>默认值</label><el-input v-model="comp.defaultValue" size="small" @input="emitUpdate" /></div>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div v-show="openSections.validation" class="section-body">
        ...
        <div v-if="isNumeric" class="prop-row">
          <label>最小值</label>
          <el-input-number v-model="numMin" size="small" @change="onNumRangeChange" />
        </div>
      </div>
    </div>
  </div>
...
const numMin = computed({
  get: () => (comp.props as Record<string, unknown>)?.min as number ?? 0,
  set: () => {},
})
...
.section-header { font-size: 13px; font-weight: 500; padding: 8px 0; cursor: pointer; user-select: none; display: flex; align-items: center; gap: 4px; border-bottom: 1px solid #F0EFEC; }
.section-arrow { font-size: 10px; color: #9C9790; transition: transform .2s; display: inline-block; }
...
.prop-row label { font-size: 12px; color: #6B6560; width: 60px; flex-shrink: 0; }
.prop-row :deep(.el-input), .prop-row :deep(.el-select) { width: 180px; }
</style>
