...
const icon = computed({
  get: () => (propsData.value?.icon as string) ?? 'star',
  set: (v) => {
    propsData.value.icon = v
    emitUpdate()
  },
})
const iconColor = computed({
  get: () => (propsData.value?.iconColor as string) ?? '#F7BA2A',
  set: (v) => {
    propsData.value.iconColor = v
    emitUpdate()
  },
})
const maxScore = ref((propsData.value?.maxScore as number) ?? 5)
const defaultScore = computed({
  get: () => (props.component.defaultValue as number) ?? 3,
  set: (v) => {
    props.component.defaultValue = v
    emitUpdate()
  },
})
